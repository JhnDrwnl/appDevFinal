<!-- components/user/EditAddress.vue -->
<template>
    <div class="max-w-4xl mx-auto bg-white">
      <h2 class="text-3xl font-bold p-6 border-b border-gray-200">Edit Shipping Address</h2>
      ...
      <form @submit.prevent="submitForm" class="p-6 space-y-6">
        ...
        <div>
          ...
          <div id="editMap" :class="['w-full h-64 mb-4 border border-gray-300 rounded', { 'hidden': !isMapLoaded }]"></div>
        ...
        <div class="flex justify-end space-x-4">
          ...
          >
            {{ isSubmitting ? 'Saving...' : 'Update Shipping Address' }}
          </button>
        </div>
      </form>
    </div>
  ...
  export default {
    ...
    setup(props, { emit }) {
      ...
      const form = ref({
        ...
        otherDetails: props.address.otherDetails || '',
      ...
      const submitForm = async () => {
        if (!authStore.isAuthenticated) {
          apiError.value = 'Please log in to update your shipping address.';
          return;
        }
  
        try {
          isSubmitting.value = true;
          const result = await addressStore.updateAddress(props.address.docId, form.value);
          
          if (result.success) {
            alert('Shipping address updated successfully!');
            emit('address-updated', { ...form.value, docId: props.address.docId });
          } else {
            apiError.value = result.error || 'Failed to update shipping address';
          }
        } catch (error) {
          console.error('Error updating shipping address:', error);
          apiError.value = 'An error occurred while updating the shipping address. Please try again.';
        } finally {
