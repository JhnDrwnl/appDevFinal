      ...
      <div class="bg-white rounded-2xl shadow-sm border border-gray-200">
        <div class="flex items-center justify-between h-16 px-6">
          ...
          <div class="flex items-center gap-4 md:gap-6">
            ...
            >
              <Bell class="w-5 h-5" />
              <span v-if="unreadCount > 0" class="absolute -top-1 -right-1 bg-[#FF9934] text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center">
                {{ unreadCount }}
              </span>
            </button>
            ...
            <div class="relative ml-auto">
              ...
              >
                <div class="px-4 py-3 border-b border-gray-50">
                  ...
                  <p class="text-xs font-medium text-gray-500 mt-1">{{ user?.role || 'Driver' }}</p>
                </div>
                <router-link
...
watch(() => reservationStore.reservations, () => {
  const newReservations = reservationStore.reservations.filter(r => r.status === 'pending' && r.type === 'delivery')
  newReservations.forEach(reservation => {
    notificationStore.addNotification({
      id: reservation.id,
      type: 'new_delivery',
      message: `New delivery assigned: ${reservation.id}`,
      read: false
    })
  })
}, { deep: true })
...
const toggleNotifications = async () => {
  await router.push({ name: 'DriverNotifications' })
