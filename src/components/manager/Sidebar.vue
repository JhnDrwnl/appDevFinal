<template>
  ...
  const menuItems = [
    ...
    { name: 'Tasks', path: '/manager/tasks', icon: ClipboardDocumentListIcon },
    { name: 'Settings', path: '/manager/settings', icon: CogIcon },
    { name: 'Profile', path: '/manager/profile', icon: UserIcon },
  ]
  ...
  watch(() => route.path, (newPath) => {
    ...
    if (newPath === '/manager/profile' || newPath === '/manager/settings') {
      openSidebar()
    }
  })
  ...
  router.afterEach((to) => {
    if (to.name === 'ManagerProfile' || to.name === 'ManagerSettings') {
      openSidebar()
    }
  })
  </script>