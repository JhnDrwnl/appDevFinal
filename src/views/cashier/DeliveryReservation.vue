<!-- views/cashier/DeliveryReservation -->
<template>
  <div class="w-full">
    <div v-if="!selectedReservation">
      <h2 class="text-2xl font-bold mb-6 text-gray-800">Delivery Reservations</h2>
      ...
      <div class="space-y-6">
        <div class="flex items-center justify-between mb-6">
          <div class="relative flex-1 max-w-md">
            <input
              type="text"
              placeholder="Search Delivery Reservation"
              v-model="searchQuery"
              ...
              <thead class="bg-gray-50">
                <tr>
                  ...
                  <th @click="sort('status')" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer">
                    Status
                    <ArrowsUpDownIcon v-if="sortColumn === 'status'" :class="{ 'transform rotate-180': sortDirection === 'desc' }" class="inline-block w-4 h-4 ml-1" />
                  </th>
                ...
                <tr v-for="reservation in paginatedReservations" :key="reservation.id">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div class="flex space-x-4">
                      ...
                      <button
                        v-if="reservation.status === 'approved' && !reservation.notifiedDrivers"
                        @click="notifyDrivers(reservation.id)"
                        class="bg-[#FF9934] hover:bg-[#FF7700] text-white font-bold py-2 px-4 rounded-full focus:outline-none focus:shadow-outline"
...
const notifyDrivers = async (reservationId) => {
  try {
    const result = await reservationStore.notifyDrivers(reservationId)
    if (result.success) {
      showAlertMessage('Drivers notified successfully')
    } else {
      showAlertMessage('Failed to notify drivers. Please try again.', 'error')
    }
  } catch (error) {
    console.error('Error notifying drivers:', error)
    showAlertMessage('An error occurred while notifying drivers', 'error')
  }
}
</script>
