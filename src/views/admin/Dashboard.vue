    ...
    <div class="mx-auto max-w-[1400px] space-y-6">
      ...
      <div class="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
        ...
        <div class="group relative overflow-hidden rounded-2xl bg-white p-6 shadow-[0_2px_8px_rgb(0,0,0,0.04)] transition-all hover:shadow-[0_4px_12px_rgb(0,0,0,0.06)]">
          <div class="flex justify-between">
            <div class="space-y-1">
              <h2 class="text-sm font-medium text-slate-600">Active Customers</h2>
              <p class="text-3xl font-bold text-slate-800">{{ customerCount }}</p>
              <div class="flex items-center gap-1">
                <span :class="customerPercentageChange >= 0 ? 'text-emerald-600' : 'text-rose-600'"
                      class="inline-flex items-center text-sm font-medium">
                  {{ customerPercentageChange >= 0 ? '+' : '' }}{{ customerPercentageChange }}%
                </span>
                <span class="text-xs text-slate-500">vs last week</span>
              </div>
            </div>
            <div class="relative h-16 w-24">
              <svg viewBox="0 0 100 50" class="h-full w-full transform transition-transform duration-300 group-hover:scale-105">
                <polyline fill="none" stroke="#FF9934" stroke-width="2" :points="getChartPoints(weeklyCustomerData)"
                          class="transition-all duration-300" />
              </svg>
            </div>
          </div>
...
const fetchCustomers = async () => {
  ...
  reservationsSnapshot.docs.forEach(doc => {
    activeCustomerIds.add(doc.data().userId)
  })

  customerCount.value = activeCustomerIds.size

  const oneWeekAgo = Timestamp.fromDate(new Date(Date.now() - 7 * 24 * 60 * 60 * 1000))
  const weeklyReservationsQuery = query(reservationsRef, where('createdAt', '>=', oneWeekAgo), orderBy('createdAt', 'asc'))
  const weeklyReservationsSnapshot = await getDocs(weeklyReservationsQuery)

  const customersByDay = weeklyReservationsSnapshot.docs.reduce((acc, doc) => {
  ...
  weeklyCustomerData.value = Array.from({ length: 7 }, (_, i) => {
    const date = new Date(Date.now() - (6 - i) * 24 * 60 * 60 * 1000).toDateString()
    return customersByDay[date] ? customersByDay[date].size : 0
  })
}
...
const calculatePercentageChanges = () => {
  ...
  customerPercentageChange.value = calculateChange(customerCount.value, customerCount.value - weeklyCustomerData.value[0])
}
